...
日本語・英語・ミャンマー語対応に加え、料理検索、おすすめ診断、カート保存、注文履歴、配達・店頭受取、スタッフ権限別の注文・在庫管理画面を追加し、ミャンマー料理店向けにデザインと注文フローをアレンジしました。
...
- `/staff` は **role-based demo management screen（権限別デモ管理画面）** です。役割別ログインで画面を保護していますが、GitHub Pages上の静的デモ認証です。本番運用ではサーバー側認証、セッション管理、権限確認を実装してください。
