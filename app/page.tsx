...
const uiText = {
  jp: {
    ...
    checkout:'CHECKOUT', deliveryInfo:'お届け情報', receiveMethod:'受け取り方法', deliveryToAddress:'ご指定の住所へ', pickupAtStore:'お店で受け取り', customerInfo:'お客様情報', nameRequired:'お名前（必須）', phoneRequired:'電話番号（必須）', addressRequired:'配達先住所（必須）', receiveTime:'受取時間', asap:'できるだけ早く', schedule:'時間指定', remarks:'備考欄（任意）', remarksPlaceholder:'アレルギーや受け取りについてのご要望', payment:'お支払い', cash:'現金', card:'カード', cardNumber:'カード番号（必須）', expiry:'有効期限（必須）', cvcRequired:'CVC（必須）', paypayHelp:'注文内容を確認した後、PayPayでのお支払い案内を表示します。', paymentTotal:'お支払い合計', reviewOrder:'注文内容を確認', notConfirmed:'このボタンではまだ注文は確定しません',
    finalReview:'FINAL REVIEW', finalTitle:'注文内容の最終確認', finalHelp:'内容をご確認のうえ、注文を確定してください。', orderItems:'ご注文', receiveInfo:'受け取り情報', method:'方法', address:'住所', name:'お名前', phone:'電話番号', backEdit:'戻って修正', placeOrder:'この内容で注文する', orderComplete:'注文が完了しました', thanks:'ご注文ありがとうございます', receipt:'レシート', orderNumber:'注文番号', issuedAt:'発行日時', total:'合計', backHome:'ホームに戻る',
...
export default function Home() {
  ...
  const validateCheckout = () => {
    if (method === 'delivery' && (!checkoutForm.name.trim() || !checkoutForm.phone.trim() || !checkoutForm.address.trim())) {
      flash(l('配達にはお名前・電話番号・住所が必要です','Delivery requires your name, phone number and address','ပို့ဆောင်ရန် အမည်၊ ဖုန်းနံပါတ်နှင့် လိပ်စာ လိုအပ်ပါသည်')); return false;
    }
    if (method === 'delivery') {
      const phoneDigits = checkoutForm.phone.replace(/\D/g, '');
      if (phoneDigits.length < 8 || phoneDigits.length > 15) {
        flash(l('正しい電話番号を入力してください','Enter a valid phone number','မှန်ကန်သော ဖုန်းနံပါတ်ကို ဖြည့်ပါ')); return false;
      }
    }
    if (checkoutForm.timing === 'scheduled' && !checkoutForm.scheduled) {
      flash(l('受取時間を指定してください','Choose a receive time','လက်ခံမည့်အချိန်ကို သတ်မှတ်ပါ')); return false;
    }
    if (checkoutForm.payment === 'card') {
      const cardDigits = checkoutForm.cardNumber.replace(/\D/g, '');
      if (cardDigits.length < 12 || cardDigits.length > 19) {
        flash(l('正しいカード番号を入力してください','Enter a valid card number','မှန်ကန်သော ကတ်နံပါတ်ကို ဖြည့်ပါ')); return false;
      }
      if (!/^(0[1-9]|1[0-2])\/\d{2}$/.test(checkoutForm.expiry.trim())) {
        flash(l('有効期限をMM/YY形式で入力してください','Enter the expiry date as MM/YY','သက်တမ်းကုန်ရက်ကို MM/YY ပုံစံဖြင့် ဖြည့်ပါ')); return false;
      }
      if (!/^\d{3,4}$/.test(checkoutForm.cvc.trim())) {
        flash(l('CVCを3〜4桁で入力してください','Enter a 3 or 4 digit CVC','CVC ကို ဂဏန်း ၃ လုံး သို့မဟုတ် ၄ လုံးဖြင့် ဖြည့်ပါ')); return false;
      }
    }
    return true;
  };
  const orderPaymentLabel = (order: OrderRecord) => {
    if (order.payment === 'cash') return t('cash');
    if (order.payment === 'card') return `${t('card')}${order.cardLast4 ? ` •••• ${order.cardLast4}` : ''} · ${t('demoPayment')}`;
    return `PayPay · ${t('demoPayment')}`;
  };
  const itemChoiceSummary = (item: CartItem) => {
    const choices = [
      choiceLabelOf(item.options.find((option) => option.id === item.selectedOption) || item.options[0], language),
      !item.hideSpice ? choiceLabelOf((item.flavors || spiceOptions).find((spice) => spice.id === item.selectedSpice) || spiceOptions[0], language) : '',
      item.secondaryOptions ? choiceLabelOf(item.secondaryOptions.find((option) => option.id === item.selectedSecondary) || item.secondaryOptions[0], language) : '',
      ...(item.multiOptions || []).filter((option) => item.selectedMulti.includes(option.id)).map((option) => choiceLabelOf(option, language)),
      ...item.toppings.filter((topping) => item.selectedToppings.includes(topping.id)).map((topping) => choiceLabelOf(topping, language)),
    ];
    return choices.filter(Boolean).join(language === 'en' ? ', ' : '・');
  };
  const placeOrder = () => {
    ...
    const record: OrderRecord = {
      ...
      cardLast4: checkoutForm.payment === 'card' ? cardDigits.slice(-4) : undefined,
      note: checkoutForm.note.trim() || undefined,
      customer: method === 'delivery' ? { name:checkoutForm.name.trim(), phone:checkoutForm.phone.trim(), address:checkoutForm.address.trim() } : undefined,
    };
    ...
    setCheckoutForm((current) => ({ ...current, cardNumber:'', expiry:'', cvc:'' }));
  };
  const openCheckoutReview = () => {
    if (validateCheckout()) setCheckoutReview(true);
  };
    ...
    <main className='app-shell'>
      ...
      {checkout && <div className='modal-layer'><div className='checkout-modal'><button className='modal-close' onClick={() => { setCheckout(false); setCheckoutReview(false); }}>×</button>
        ...
        {!checkoutReview ? <div className='checkout-grid'><div><h3>1. {t('receiveMethod')}</h3><div className='method-switch checkout-method'><button className={method === 'delivery' ? 'active' : ''} onClick={() => setMethod('delivery')}><span>◈</span><div><b>{t('delivery')}</b><small>{t('deliveryToAddress')}</small></div></button><button className={method === 'pickup' ? 'active' : ''} onClick={() => setMethod('pickup')}><span>▣</span><div><b>{t('pickup')}</b><small>{t('pickupAtStore')}</small></div></button></div>
          {method === 'delivery' && <><h3>2. {t('customerInfo')}</h3><div className='form-grid'><label><span>{t('nameRequired')}</span><input required autoComplete='name' value={checkoutForm.name} onChange={(e) => setCheckoutForm({...checkoutForm,name:e.target.value})} placeholder={l('例）アイ・タンダー','Example: Aye Thandar','ဥပမာ) Aye Thandar')}/></label><label><span>{t('phoneRequired')}</span><input required type='tel' autoComplete='tel' value={checkoutForm.phone} onChange={(e) => setCheckoutForm({...checkoutForm,phone:e.target.value})} placeholder='090-1234-5678'/></label><label className='full'><span>{t('addressRequired')}</span><input required autoComplete='street-address' value={checkoutForm.address} onChange={(e) => setCheckoutForm({...checkoutForm,address:e.target.value})} placeholder='沖縄県那覇市泉崎 1-1-1'/></label></div></>}
          ...
          {checkoutForm.payment === 'card' && <div className='payment-fields'><label><span>{t('cardNumber')}</span><input required inputMode='numeric' autoComplete='cc-number' maxLength={23} value={checkoutForm.cardNumber} onChange={(e) => setCheckoutForm({...checkoutForm,cardNumber:e.target.value})} placeholder='1234 5678 9012 3456'/></label><div><label><span>{t('expiry')}</span><input required inputMode='numeric' autoComplete='cc-exp' maxLength={5} value={checkoutForm.expiry} onChange={(e) => setCheckoutForm({...checkoutForm,expiry:e.target.value})} placeholder='MM/YY'/></label><label><span>{t('cvcRequired')}</span><input required inputMode='numeric' autoComplete='cc-csc' maxLength={4} value={checkoutForm.cvc} onChange={(e) => setCheckoutForm({...checkoutForm,cvc:e.target.value})} placeholder='123'/></label></div></div>}
          ...
          <div className='checkout-review'><p className='eyebrow'>{t('finalReview')}</p><h2>{t('finalTitle')}</h2><p>{t('finalHelp')}</p><div className='review-summary'><section><h3>{t('orderItems')}</h3><div className='review-items'>{cart.map((item, index) => <p key={item.id + '-review-' + index}><span>{foodNameOf(item, language)} × {item.quantity}</span><b>{yen(item.unitPrice * item.quantity)}</b></p>)}</div><p><span>{t('productTotal')}</span><b>{yen(subtotal)}</b></p>{couponDiscount > 0 && <p className='discount-row'><span>{t('discount')}</span><b>−{yen(couponDiscount)}</b></p>}<p><span>{t('deliveryFee')}</span><b>{fee ? yen(fee) : t('free')}</b></p><div><span>{t('paymentTotal')}</span><strong>{yen(paymentTotal)}</strong></div></section><section><h3>{t('receiveInfo')}</h3><p><span>{t('method')}</span><b>{method === 'delivery' ? t('delivery') : t('pickup')}</b></p>{method === 'delivery' && <><p><span>{t('address')}</span><b>{checkoutForm.address}</b></p><p><span>{t('name')}</span><b>{checkoutForm.name}</b></p><p><span>{t('phone')}</span><b>{checkoutForm.phone}</b></p></>}<p><span>{t('receiveTime')}</span><b>{checkoutForm.timing === 'asap' ? t('asap') : checkoutForm.scheduled}</b></p><p><span>{t('payment')}</span><b>{checkoutForm.payment === 'cash' ? t('cash') : checkoutForm.payment === 'card' ? `${t('card')} •••• ${checkoutForm.cardNumber.replace(/\D/g, '').slice(-4)} · ${t('demoPayment')}` : `PayPay · ${t('demoPayment')}`}</b></p>{checkoutForm.note && <p><span>{t('note')}</span><b>{checkoutForm.note}</b></p>}</section></div><div className='review-actions'><button className='secondary' onClick={() => setCheckoutReview(false)}>{t('backEdit')}</button><button className='primary' onClick={placeOrder}>{yen(paymentTotal)}　{t('placeOrder')}</button></div></div>}
      </div></div>}

      {tracking && completedOrder && <div className='modal-layer'><div className='tracking-card receipt-card'><button className='modal-close' onClick={() => setTracking(false)}>×</button><div className='success-icon'>✓</div><p className='eyebrow'>ORDER #{completedOrder.id}</p><h2>{completedOrder.status === '完了' ? t('orderComplete') : t('trackingTitle')}</h2><p>{t('thanks')}</p><div className='order-progress'>{(completedOrder.method === 'delivery' ? ['受付','調理中','配達中','完了'] : ['受付','調理中','受取待ち','完了']).map((status, index, steps) => { const current = Math.max(0, steps.indexOf(completedOrder.status)); return <div className={index <= current ? 'active' : ''} key={status}><span>{index < current ? '✓' : index + 1}</span><b>{status === '受付' ? t('accepted') : status === '調理中' ? t('cooking') : status === '配達中' ? t('delivering') : status === '受取待ち' ? t('readyPickup') : t('completed')}</b></div>; })}</div><div className='eta'><small>{completedOrder.method === 'delivery' ? 'DELIVERY TIME' : 'PICKUP TIME'}</small><strong>{completedOrder.receiveTime}</strong></div><section className='receipt'><div className='receipt-heading'><p className='eyebrow'>{t('receipt')}</p><strong>HOME MYANMAR RESTAURANT</strong></div><div className='receipt-meta'><p><span>{t('orderNumber')}</span><b>#{completedOrder.id}</b></p><p><span>{t('issuedAt')}</span><b>{completedOrder.createdAt}</b></p></div><div className='complete-items'>{completedOrder.items.map((item,index) => <div className='receipt-item' key={item.id + '-' + index}><p><span>{foodNameOf(item, language)} × {item.quantity}</span><b>{yen(item.unitPrice * item.quantity)}</b></p><small>{itemChoiceSummary(item)}</small>{item.note && <small>{t('note')}: {item.note}</small>}</div>)}</div><div className='receipt-summary'><p><span>{t('productTotal')}</span><b>{yen(completedOrder.subtotal)}</b></p>{completedOrder.discount > 0 && <p className='discount-row'><span>{t('discount')}</span><b>−{yen(completedOrder.discount)}</b></p>}<p><span>{t('deliveryFee')}</span><b>{completedOrder.deliveryFee ? yen(completedOrder.deliveryFee) : t('free')}</b></p></div><div className='receipt-info'><p><span>{t('method')}</span><b>{completedOrder.method === 'delivery' ? t('delivery') : t('pickup')}</b></p>{completedOrder.customer && <><p><span>{t('name')}</span><b>{completedOrder.customer.name}</b></p><p><span>{t('phone')}</span><b>{completedOrder.customer.phone}</b></p><p><span>{t('address')}</span><b>{completedOrder.customer.address}</b></p></>}<p><span>{t('receiveTime')}</span><b>{completedOrder.receiveTime}</b></p><p><span>{t('payment')}</span><b>{orderPaymentLabel(completedOrder)}</b></p>{completedOrder.note && <p><span>{t('note')}</span><b>{completedOrder.note}</b></p>}</div><div className='complete-total'><span>{t('paymentTotal')}</span><strong>{yen(completedOrder.total)}</strong></div></section><button className='secondary wide' onClick={() => setTracking(false)}>{t('backHome')}</button></div></div>}
